...
cvar_t  *r_ext_occlusion_query_boolean;
...
byte *RB_ReadPixels(int x, int y, int width, int height, size_t *offset, int *padlen)
{
	...
	if (qglesMajorVersion >= 1) {
		if (readFormatAvailable) {
			qglGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_FORMAT, &format);
			qglGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_TYPE, &type);
		} else {
			format = GL_RGBA;
...
void R_Register( void ) 
{
	...
	r_ext_occlusion_query_boolean = ri.Cvar_Get( "r_ext_occlusion_query_boolean", "1", CVAR_ARCHIVE | CVAR_LATCH);
